...
func TestDataValidation(t *testing.T) {
	...
	// Helper function to create a test database
	func setupTestDB(t testing.TB) (*database.Queries, func()) {
		// Create a temporary test database
		testDB, err := sql.Open("sqlite3", ":memory:")
		require.NoError(t, err)

		// Initialize schema
		_, err = testDB.ExecContext(context.Background(), database.Schema)
		require.NoError(t, err)

		// Create queries instance
		q := database.New(testDB)
	...
	func TestHandleData(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				// Insert test data if needed
				if tt.initialCash > 0 {
					_, err := q.CreatePortfolio(ctx, database.CreatePortfolioParams{
						Cash:      tt.initialCash,
						Timestamp: time.Now(),
					})
					require.NoError(t, err)
				...
				var response []database.PortfolioData
				err := json.NewDecoder(w.Body).Decode(&response)
				require.NoError(t, err)
				assert.Equal(t, tt.expectedCount, len(response))
	...
	func TestErrorConditions(t *testing.T) {
		...
		t.Run("database error", func(t *testing.T) {
			// Create an invalid database connection
			testDB, err := sql.Open("sqlite3", ":memory:") // New connection that won't have tables
			require.NoError(t, err)
			defer testDB.Close()
